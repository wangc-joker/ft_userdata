...
class Top9RegimeMainExhaustionProbeTunedStrategy(Top9RegimeMainExhaustionProbeStrategy):
    """
    Tuned version of the exhaustion breakout probe:
    - keep the breakout branch narrow
    - still allow the ZEC April 2026 breakout shape
    - filter out most weak closes / messy bases seen in losing samples
    """
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["breakout_close_near_high"] = (
            (dataframe["high"] - dataframe["close"]) / candle_range < 0.26
        )
        dataframe["breakout_body_strength"] = (
            (dataframe["close"] - dataframe["open"]) / dataframe["close"] > 0.022
        )
        dataframe["breakout_4h_shift_strong"] = (
            dataframe["trade_center_4h_proxy"] > dataframe["trade_center_4h_proxy"].shift(6) * 1.008
        )
        dataframe["breakout_base_clean"] = (
            dataframe["compression_range_8"] < 0.065
        ) & (
            dataframe["trade_floor_6"] > dataframe["trade_floor_prev_6"] * 1.001
        )
